...
const AllPartners = () => {
  ...
  const handleFetch = async () => {
    try {
      const res = await axios.get('http://localhost:7000/api/v1/get-all-company-logos');
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`http://localhost:7000/api/v1/delete-company-logo/${selectedLogoId}`);
      handleFetch();
  ...
  const handleUpload = async () => {
    try {
      ...
      await axios.post('http://localhost:7000/api/v1/upload-logo-of-Company', formData);
      handleFetch();
